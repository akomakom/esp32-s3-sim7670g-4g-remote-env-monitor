//
// ESP-NOW wire protocol shared with the hot tub controller.
//
// This is a VERBATIM copy of hot_tub_types.h from the esp32-spa-controller
// project. It MUST stay byte-for-byte identical to the controller's copy so the
// ESP-NOW payloads memcpy correctly on both ends. In particular `time_t` width
// (8 bytes on ESP32 Arduino core 3.x / IDF 5.x) determines the offset of
// `water_temp`; both projects build against that core, so the layout matches.
// Do NOT "clean up" or reorder these structs.
//
